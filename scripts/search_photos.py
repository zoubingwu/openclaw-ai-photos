...
def main():
    ...
    if args.recent and any([args.date, args.text, args.tag]):
        ...
        sys.exit(2)
    if args.recent:
        runner(args.target, f"SELECT file_path, filename, taken_at, indexed_at, caption, tags FROM photos ORDER BY indexed_at DESC, created_at DESC LIMIT {args.limit};")
        return
    where = []
    if args.date:
        if args.backend == 'db9':
    ...
    clause = ' AND '.join(where) if where else ('true' if args.backend == 'db9' else '1=1')
    runner(args.target, f"SELECT file_path, filename, taken_at, caption, tags FROM photos WHERE {clause} ORDER BY COALESCE(taken_at, created_at) DESC LIMIT {args.limit};")
